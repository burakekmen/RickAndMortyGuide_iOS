...
open class PopTip: UIView {
    ...
    open func show(customView: UIView, direction: PopTipDirection, in view: UIView, from frame: CGRect, duration: TimeInterval? = nil) {
        ...
        ///   - duration: Optional time interval that determines when the poptip will self-dismiss.
        @available(iOS 13.0, *)
        open func show<V: View>(rootView: V, direction: PopTipDirection, in tView: UIView, from frame: CGRect, parent: UIViewController, duration: TimeInterval? = nil) {
            resetView()
...
// Creator
extension PopTip {
    func setDefaultAppereancePopup() {
        bubbleColor = .white
